...
class ControllerSaleOrder extends Controller {
  		...
  		$this->document->setTitle($this->language->get('heading_title'));
			...
			if(isset($this->request->post['selected'])) {
		 	...
		 	foreach ($this->request->post['selected'] as $order_id) {
				$order_status =	$this->model_sale_order->getStatusByOrderId($order_id);
					if($order_status == 25){
					  $this->model_sale_order->processarPedido($order_id);
						$guia = '';
						$guia = $this->url->link('sale/order/invoice', 'token=' .
						$this->session->data['token']. $url.'&order_id='.$order_id);

						$guia = str_replace("&amp;", "&", $guia);
						echo "<script type=\"text/javascript\">
						window.open('".$guia."', '_blank')  </script>";
					}
				}
							...
							if (isset($this->request->get['filter_customer'])) {
								$url .= '&filter_customer=' .
								urlencode(html_entity_decode($this->request->get['filter_customer'],
								ENT_QUOTES, 'UTF-8'));
							}

							if (isset($this->request->get['filter_order_status_id'])) {
								$url .= '&filter_order_status_id=' .
								$this->request->get['filter_order_status_id'];
							}

							if (isset($this->request->get['filter_shipping_method'])) {
								$url .= '&filter_shipping_method=' .
								$this->request->get['filter_shipping_method'];
							}
							if (isset($this->request->get['filter_delivery_date'])) {
								$url .= '&filter_delivery_date=' .
								$this->request->get['filter_delivery_date'];
							}

							if (isset($this->request->get['filter_delivery_time'])) {
								$url .= '&filter_delivery_time=' .
								$this->request->get['filter_delivery_time'];
							}

							if (isset($this->request->get['filter_total'])) {
								$url .= '&filter_total=' .
								$this->request->get['filter_total'];
							}

							if (isset($this->request->get['filter_date_added'])) {
								$url .= '&filter_date_added=' .
								$this->request->get['filter_date_added'];
							}

							if (isset($this->request->get['filter_date_modified'])) {
								$url .= '&filter_date_modified=' .
								$this->request->get['filter_date_modified'];
							}
						...
						// 	print_r($guia);
						//  die;
						//  $this->redirect($this->url->link('sale/order/invoice', 'token=' .
					  //  $this->session->data['token']. $url.'&order_id='.$order_id));
	...
	protected function getList() {
		...
		$this->data['relatorio'] = $this->url->link('sale/relatorio', 'token=' . $this->session->data['token'], 'SSL');
		...
		// $this->data['processapedido'] = $this->url->link('sale/order/processapedido', 'token=' .$this->session->data['token'] . $url, 'SSL');
		$this->data['processapedido'] = $this->url->link('sale/order/processapedido', 'token=' .$this->session->data['token'], 'SSL');
		...
		$this->data['sort_delivery_date'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=o.ddw_delivery_date' . $url, 'SSL');
		$this->data['sort_delivery_time'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=o.ddw_time_Slot' . $url, 'SSL');
		...
		$this->data['filter_delivery_time'] = $filter_delivery_time;
		...
		$this->data['shipping_method'] = $this->model_localisation_order_status->getOrderMethods();
		$this->data['ddelivery_time'] = $this->model_localisation_order_status->getOrderDTimes();
...
$this->data['text_view_btn'] = $this->language->get('text_view_btn');
...
$this->data['view_order'] = "";
			$this->data['action'] = $this->url->link('sale/order/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
$this->data['view_order'] = $this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . $url, 'SSL');
		...
		if (isset($this->request->post['filter_shipping_method'])) {
			$this->data['filter_shipping_method'] = $this->request->post['filter_shipping_method'];
		} elseif (!empty($order_info)) {
			$this->data['filter_shipping_method'] = $order_info['shipping_method'];
		} else {
			$this->data['filter_shipping_method'] = '';
		}
		if (isset($this->request->post['filter_delivery_date'])) {
			$this->data['filter_delivery_date'] = $this->request->post['filter_delivery_date'];
		} elseif (!empty($order_info)) {
			if(!isset($order_info['ddw_delivery_date'])){
				$order_info['ddw_delivery_date']="";
			}
			$this->data['filter_delivery_date'] = $order_info['ddw_delivery_date'];
		} else {
			$this->data['filter_delivery_date'] = '';
		}

		if (isset($this->request->post['filter_delivery_time'])) {
			$this->data['filter_delivery_time'] = $this->request->post['filter_delivery_time'];
		} elseif (!empty($order_info)) {
			if(!isset($order_info['ddw_time_slot'])){
				$order_info['ddw_time_slot']="";
			}
			$this->data['filter_delivery_time'] = $order_info['ddw_time_slot'];
		} else {
			$this->data['filter_delivery_time'] = '';
		}
		...
		if (isset($this->request->post['ddelivery_time'])) {
			$this->data['ddelivery_time'] = $this->request->post['ddelivery_time'];
		} elseif (!empty($order_info)) {
			$this->data['ddelivery_time'] = $order_info['ddw_time_slot'];
		} else {
			$this->data['ddelivery_time'] = '';
		}
...
$this->data['text_edit_btn'] = $this->language->get('text_edit_btn');
...
$this->data['edit_order'] = $this->url->link('sale/order/update', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . $url, 'SSL');
			$this->data['cancel'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	public function invoice($orders=null) {
		$this->language->load('sale/order');
		...
		foreach ($orders as $order_id) {
			...
			$this->data['column_image'] = $this->language->get('column_image');
			$image = $this->model_sale_order->getProductImageInvoice($product['product_id']);
					$this->load->model('tool/image');
					if ($image && file_exists(DIR_IMAGE . $image)) {
						$image = $this->model_tool_image->resize($image, 80, 80);
					} else {
			...
			'image'    	 	   => $image,
				...
				$this->data['orders'][] = array(
					...
					'total'              => $total_data,
					'ddw_delivery_date'  => date($this->language->get('date_format_short'),
					strtotime($order_info['ddw_delivery_date'])),
					'ddw_time_slot'  => $order_info['ddw_time_slot'],
