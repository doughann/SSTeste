...
class ModelSaleOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['order_product'])) {
			foreach ($data['order_product'] as $order_product) {
				...
				for($i = $order_product['quantity']; $i>0; $i--){//wagner - retirando do estoque os produtos
		...
		//*ATENÇAO, se a regra mudar, mudar tambem no catalog/model/checkout/order.php
			...
			$queryinsere = "INSERT INTO " . DB_PREFIX . "customer_reward (customer_id, order_id, description, points, ativo, date_added) values('".$data['customer_id']."', '".$order_id."', 'Pontos do pedido n° #".$order_id."', '".$pontos_recompensa."', '0', NOW())";
	...
	{
		$order_query = $this->db->query("SELECT order_status_id FROM `" . DB_PREFIX .
		...
		return $order_query->row['order_status_id'];
			...
			public function getProductImageInvoice($product_id){
				$query = $this->db->query("select image as image from " . DB_PREFIX . "product where product_id = ".(int)$product_id);
				return $query->row['image'];
			}
	...
	public function editOrder($order_id, $data) {
		...
		"order` WHERE order_status_id > '0' AND order_id = '" . (int)$order_id . "'");

		if ($order_query->num_rows) {
			$product_query = $this->db->query("SELECT * FROM " . DB_PREFIX .
			"order_product WHERE order_id = '" . (int)$order_id . "'");

			foreach($product_query->rows as $product) {
				$this->db->query("UPDATE `" . DB_PREFIX . "product` SET quantity =
				(quantity + " . (int)$product['quantity'] . ") WHERE product_id = '" .
				(int)$product['product_id'] . "' AND subtract = '1'");

				//wagner - adicionando ao estoque os produtos
					$this->db->query("insert into " . DB_PREFIX . "product_stock (stock_qty,
					stock_product_id) values('".(int)$product['quantity']."', '" .
					(int)$product['product_id'] . "')");


				$option_query = $this->db->query("SELECT * FROM " . DB_PREFIX .
				"order_option WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" .
				(int)$product['order_product_id'] . "'");

				foreach ($option_query->rows as $option) {
					$this->db->query("UPDATE " . DB_PREFIX .
					"product_option_value SET quantity = (quantity + " .
					 (int)$product['quantity'] . ") WHERE product_option_value_id = '" .
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET firstname = '" .
		 ...
		 $this->db->escape($data['fax']) . "', payment_firstname = '" .
		 $this->db->escape($data['payment_firstname']) . "', payment_lastname = '" .
	  ...
	  $this->db->escape($data['payment_address_2']) . "', payment_city = '" .
		$this->db->escape($data['payment_city']) . "', payment_postcode = '" .
		$this->db->escape($data['payment_postcode']) . "', payment_country = '" .
		$this->db->escape($payment_country) . "', payment_country_id = '" .
		(int)$data['payment_country_id'] . "', payment_zone = '" .
		$this->db->escape($payment_zone) . "', payment_zone_id = '" .
		(int)$data['payment_zone_id'] . "', payment_address_format = '" .
		$this->db->escape($payment_address_format) . "', payment_method = '" .
		$this->db->escape($data['payment_method']) . "', payment_code = '" .
		...
		$this->db->escape($data['shipping_address_2']) . "', shipping_city = '" .
		$this->db->escape($data['shipping_city']) . "', shipping_postcode = '" .
		$this->db->escape($data['shipping_postcode']) . "', shipping_country = '" .
		$this->db->escape($shipping_country) . "', shipping_country_id = '" .
		(int)$data['shipping_country_id'] . "', shipping_zone = '" .
		$this->db->escape($shipping_zone) . "', shipping_zone_id = '" .
		(int)$data['shipping_zone_id'] . "', shipping_address_format = '" .
		$this->db->escape($shipping_address_format) . "', shipping_method = '" .
		$this->db->escape($data['shipping_method']) . "', shipping_code = '" .
		$this->db->escape($data['shipping_code']) . "', comment = '" .
		$this->db->escape($data['comment']) . "', order_status_id = '" .
		(int)$data['order_status_id'] . "', affiliate_id  = '" .
		(int)$data['affiliate_id'] . "', date_modified = NOW() WHERE order_id = '" .
		(int)$order_id . "'");

		//$data['order_status_id']
		//$order_id
		if($data['order_status_id']=='2' or $data['order_status_id']=='3' or
		$data['order_status_id']=='5' or $data['order_status_id']=='15' or
		$data['order_status_id']=='17' or $data['order_status_id']=='18' or
		$data['order_status_id']=='19' or $data['order_status_id']=='25'){
			$ativar='1';
		}else{
			$ativar='0';
		}
		$this->db->query("UPDATE " . DB_PREFIX . "customer_reward set ativo='".
		$ativar."' WHERE order_id = '" . (int)$order_id . "'");
		...
		if (isset($data['order_product'])) {
			foreach ($data['order_product'] as $order_product) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_product_id = '" .
				(int)$order_product['order_product_id'] . "', order_id = '" .
				(int)$order_id . "', product_id = '" . (int)$order_product['product_id'] .
				 "', name = '" . $this->db->escape($order_product['name']) . "', model = '" .
				 $this->db->escape($order_product['model']) . "', quantity = '" .
				 (int)$order_product['quantity'] . "', price = '" .
				 (float)$order_product['price'] . "', total = '" .
				 (float)$order_product['total'] . "', tax = '" .
				 (float)$order_product['tax'] . "', reward = '" .
				 (int)$order_product['reward'] . "'");
				...
				for($i = $order_product['quantity']; $i>0; $i--){//wagner - retirando do estoque os produtos
					...
					DB_PREFIX . "product_stock SET stock_qty = (stock_qty - 1) WHERE stock_product_id = '" .
					(int)$order_product['product_id'] . "' AND stock_qty > '0' order by lot_exp ASC limit 1");
				}

				if (isset($order_product['order_option'])) {
					foreach ($order_product['order_option'] as $order_option) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_option_id = '" .
						(int)$order_option['order_option_id'] . "', order_id = '" .
						(int)$order_id . "', order_product_id = '" .
						(int)$order_product_id . "', product_option_id = '" .
						(int)$order_option['product_option_id'] . "', product_option_value_id = '" .
						(int)$order_option['product_option_value_id'] . "', name = '" .
						$this->db->escape($order_option['name']) . "', `value` = '" .
					  $this->db->escape($order_option['value']) . "', `type` = '" .
						$this->db->escape($order_option['type']) . "'");


						$this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET quantity =
						(quantity - " . (int)$order_product['quantity'] . ") WHERE product_option_value_id = '" .
				...
				if (isset($order_product['order_download'])) {
					foreach ($order_product['order_download'] as $order_download) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "order_download SET order_download_id = '" .
						(int)$order_download['order_download_id'] . "', order_id = '" .
						(int)$order_id . "', order_product_id = '" .
						(int)$order_product_id . "', name = '" .
						$this->db->escape($order_download['name']) .
						"', filename = '" .
						$this->db->escape($order_download['filename']) .
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET total = '" .
		(float)$total . "', affiliate_id = '" . (int)$affiliate_id .
		"', commission = '" . (float)$commission . "' WHERE order_id = '" .
		(int)$order_id . "'");



		//*ATENÇAO, se a regra mudar, mudar tambem no admin/model/sale/order.php
			$querytotais = "select * from " . DB_PREFIX .
			"order_total where order_id='".$order_id."' and code='sub_total'";
			$buscatotais = $this->db->query($querytotais);

			$total_produtos = $buscatotais->row['value'];
			$queryship = "select * from " . DB_PREFIX .
			"order_total where order_id='".$order_id."' and code='shipping'";
			$buscaship = $this->db->query($queryship);
			...
			//oc_order - customer_group_id
			//oc_customer_group - reward_percentage
			$querygrupo="select " . DB_PREFIX . "order.customer_group_id,
			oc_customer_group.reward_percentage as rpercent from oc_order,
			oc_customer_group  where order_id='".$order_id."' and
			oc_order.customer_group_id = oc_customer_group.customer_group_id";
			$buscagrupo = $this->db->query($querygrupo);
			...
			//oc_reward_interval
			$queryintervalo="select * from " . DB_PREFIX .
			"reward_interval where valor_inicial<='".$total_produtos.
			"' and valor_final>='".$total_produtos."'";
			$buscaintervalo = $this->db->query($queryintervalo);//nao inclui frete
			if ($buscaintervalo->num_rows) {
				$pontos_intervalo = $buscaintervalo->row['pontos'];
			}else{
			...
			//$order_info['customer_id']
			$queryinsere = "INSERT INTO " . DB_PREFIX . "customer_reward (customer_id, order_id,
			description, points, ativo, date_added) values('".$data['customer_id']."',
			'".$order_id."', 'Pontos do pedido n° #".$order_id."', '".$pontos_recompensa."', '0', NOW())";
	...
	public function deleteOrder($order_id) {
		$order_query = $this->db->query("SELECT * FROM `" .
		DB_PREFIX . "order` WHERE order_status_id > '0' AND order_id = '" .
		(int)$order_id . "'");

		if ($order_query->num_rows) {
			$product_query = $this->db->query("SELECT * FROM " .
			DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");

			foreach($product_query->rows as $product) {
				$this->db->query("UPDATE `" . DB_PREFIX .
				"product` SET quantity = (quantity + " . (int)$product['quantity'] . ")
				WHERE product_id = '" . (int)$product['product_id'] . "' AND subtract = '1'");
				//wagner - adicionando ao  estoque os produtos
				$this->db->query("insert into " . DB_PREFIX .
				"product_stock (stock_qty, stock_product_id) values('".(int)$product['quantity']."', '" .
				(int)$product['product_id'] . "')");

				$option_query = $this->db->query("SELECT * FROM " .
				 DB_PREFIX . "order_option WHERE order_id = '" .
	...
	public function getOrder($order_id) {
		$order_query = $this->db->query("SELECT *, (SELECT CONCAT(c.firstname, ' ',
			 c.lastname) FROM " . DB_PREFIX . "customer c WHERE c.customer_id = o.customer_id)
			 AS customer FROM `" . DB_PREFIX . "order` o WHERE o.order_id = '" . (int)$order_id . "'");

		if ($order_query->num_rows) {
			$reward = 0;

			$order_product_query = $this->db->query("SELECT * FROM " .
			DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'".'order by name asc');

			foreach ($order_product_query->rows as $product) {
				$reward += $product['reward'];
			}

			$country_query = $this->db->query("SELECT * FROM `" .
			 DB_PREFIX . "country` WHERE country_id = '" .
			 (int)$order_query->row['payment_country_id'] . "'");
			...
			$zone_query = $this->db->query("SELECT * FROM `" .
			DB_PREFIX . "zone` WHERE zone_id = '" . (int)$order_query->row['payment_zone_id'] . "'");
			...
			$country_query = $this->db->query("SELECT * FROM `" .
			DB_PREFIX . "country` WHERE country_id = '" .
			(int)$order_query->row['shipping_country_id'] . "'");
		...
		//	print_r($order_query->row['ddw_time_slot']);
		//	die
			...
			return array(
				...
				'ddw_delivery_date'       => $order_query->row['ddw_delivery_date'],
			//	'ddw_delivery_time'       => $order_query->row['ddw_delivery_time'],
				'ddw_time_slot'           => $order_query->row['ddw_time_slot']
			);
		} else {
			return false;
		}
	...
	public function getOrders($data = array()) {
		...
		(SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id =
		o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') .
		 "') AS status,(SELECT os.cor FROM " . DB_PREFIX .
		 "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" .
		 (int)$this->config->get('config_language_id') . "') AS cor, o.total,
		 o.currency_code, o.currency_value, o.date_added, o.date_modified,
		 o.ddw_delivery_date, o.ddw_time_slot,
	   o.payment_method, o.shipping_method FROM `" . DB_PREFIX . "order` o";
		...
		if (!empty($data['filter_customer'])) {
			$sql .= " AND CONCAT(o.firstname, ' ', o.lastname) LIKE '%" .
			$this->db->escape($data['filter_customer']) . "%'";
		}

		if (!empty($data['filter_date_added'])) {
			$sql .= " AND DATE(o.date_added) = DATE('" .
			$this->db->escape($data['filter_date_added']) . "')";
		}

		if (!empty($data['filter_date_modified'])) {
			$sql .= " AND DATE(o.date_modified) = DATE('" .
		...
		if (!empty($data['filter_shipping_method'])) {
			$sql .= " AND o.shipping_method = '" . $data['filter_shipping_method'] . "'";
		}
		if (!empty($data['filter_delivery_date'])) {
			$sql .= " AND o.ddw_delivery_date = '" . $data['filter_delivery_date'] . "'";
		}
		if (!empty($data['filter_delivery_time'])) {
			$sql .= " AND o.ddw_time_slot = '" . $data['filter_delivery_time'] . "'";
		}
	...
	public function getOrderOption($order_id, $order_option_id) {
		$query = $this->db->query("SELECT * FROM " .
		DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id .
	...
	public function getOrderOptions($order_id, $order_product_id) {
		$query = $this->db->query("SELECT oo.* FROM " . DB_PREFIX .
		"order_option AS oo LEFT JOIN " . DB_PREFIX .
		"product_option po USING(product_option_id) LEFT JOIN `" .
		DB_PREFIX . "option` o USING(option_id) WHERE order_id = '" .
		(int)$order_id . "' AND order_product_id = '" . (int)$order_product_id .
	...
	public function getOrderDownloads($order_id, $order_product_id) {
		...
		"order_download WHERE order_id = '" . (int)$order_id .
	...
	public function getOrderVoucherByVoucherId($voucher_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX .
		 "order_voucher` WHERE voucher_id = '" . (int)$voucher_id . "'");
	...
	public function getOrderTotals($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX .
		"order_total WHERE order_id = '" . (int)$order_id . "' ORDER BY sort_order");
	...
	public function getTotalOrders($data = array()) {
		...
		&& !is_null($data['filter_order_status_id'])) {
		...
		if (!empty($data['filter_total'])) {
			$sql .= " AND shipping_method = '" . $data['filter_shipping_method'] . "'";
		}
	...
	public function getTotalOrdersByStoreId($store_id) {
		...
		DB_PREFIX . "order` WHERE store_id = '" . (int)$store_id . "'");
	...
	public function getTotalOrdersByOrderStatusId($order_status_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" .
		DB_PREFIX . "order` WHERE order_status_id = '" .
		(int)$order_status_id . "' AND order_status_id > '0'");
	...
	public function getTotalOrdersByLanguageId($language_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" .
		DB_PREFIX . "order` WHERE language_id = '" .
		(int)$language_id . "' AND order_status_id > '0'");
	...
	public function getTotalOrdersByCurrencyId($currency_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" .
		DB_PREFIX . "order` WHERE currency_id = '" . (int)$currency_id .
	...
	public function getTotalSales() {
		$query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX .
	...
	public function getTotalSalesByYear($year) {
		...
		DB_PREFIX . "order` WHERE order_status_id > '0' AND YEAR(date_added) = '" .
		(int)$year . "'");
	...
	public function createInvoiceNo($order_id) {
		...
		if ($order_info && !$order_info['invoice_no']) {
			$query = $this->db->query("SELECT MAX(invoice_no) AS invoice_no FROM `" .
			 DB_PREFIX . "order` WHERE invoice_prefix = '" .
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET invoice_no = '" .
			(int)$invoice_no . "', invoice_prefix = '" .
			$this->db->escape($order_info['invoice_prefix']) .
	...
	public function processarPedido($order_id) {
		...
		order_id = '" . (int)$order_id . "', order_status_id = '" .
		2 . "', notify = '" .
		(isset($data['notify']) ? (int)$data['notify'] : 0) . "', date_added = NOW()");
	...
	public function addOrderHistory($order_id, $data) {
			...
			//$data['order_status_id']
			//$order_id
			if($data['order_status_id']=='2' or
			  $data['order_status_id']=='3' or
				$data['order_status_id']=='5' or
				$data['order_status_id']=='15' or
				$data['order_status_id']=='17' or
				$data['order_status_id']=='18' or
				$data['order_status_id']=='19' or
				$data['order_status_id']=='25'){
				$ativar='1';
			}else{
				$ativar='0';
			}
			...
			(isset($data['notify']) ? (int)$data['notify'] : 0) . "', comment = '" .
			$this->db->escape(strip_tags($data['comment'])) . "', date_added = NOW()");
	...
	public function getOrderHistories($order_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT oh.date_added, os.name AS status,
			oh.comment, oh.notify FROM " . DB_PREFIX . "order_history oh LEFT JOIN " .
			 DB_PREFIX . "order_status os ON oh.order_status_id =
			 os.order_status_id WHERE oh.order_id = '" . (int)$order_id .
	...
	public function getTotalOrderHistories($order_id) {
		...
		"order_history WHERE order_id = '" . (int)$order_id . "'");
	...
	public function getTotalOrderHistoriesByOrderStatusId($order_status_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX .
		"order_history WHERE order_status_id = '" . (int)$order_status_id . "'");
	...
	public function getEmailsByProductsOrdered($products, $start, $end) {
		...
		$query = $this->db->query("SELECT DISTINCT email FROM `" . DB_PREFIX .
		"order` o LEFT JOIN " . DB_PREFIX . "order_product op ON
		(o.order_id = op.order_id) WHERE (" . implode(" OR ", $implode) . ")
		AND o.order_status_id <> '0' LIMIT " . (int)$start . "," . (int)$end);
