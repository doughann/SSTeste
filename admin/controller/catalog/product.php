...
class ControllerCatalogProduct extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_expire'])) {//wagner - filtro vencimento
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_expire'])) {//wagner - filtro de vencimento
			$url .= '&filter_expire=' . $this->request->get['filter_expire'];
		}
		...
		foreach ($results as $result) {
			...
			//$resultsqty = $this->model_catalog_product->getProductQty($result['product_id']);//wagner
			//$resultsStock = $this->model_catalog_product->getProductStock($result['product_id']);//wagner
			//$this->data['stock_lot'] = $resultsStock; //wagner quantidade lotes
			...
			//echo "AAAAAAAAAAAAAAAAAAAAAAAA";
				...
				foreach ($data['stock_lot'] as $stock_lot) {
					...
					$venc= date('Y-m-d',strtotime($stock_lot['datalot']." + ".$result['expire']." days"));
			...
			$this->data['products'][] = array(
				...
				'stock_qtd'   => $result['stock_qtd'],
				'expire'     => $result['expire'],
				'tipo'     => $result['tipo'],
				'stock_lot'   => $this->model_catalog_product->getProductStock($result['product_id']),//wagner
		...
		$this->data['sort_tipo'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.tipo' . $url, 'SSL');
	...
	protected function getForm() {
		...
		if(isset($product_info)){
			$resultsqty = $this->model_catalog_product->getProductQty($product_info['product_id']);//wagner
			$resultsStock = $this->model_catalog_product->getProductStock($product_info['product_id']);//wagner
			$this->data['quantity_original'] = $this->data['quantity'];//wagner quantidade original
			$this->data['quantity'] = $resultsqty['qtd'];//wagner quantidade master
			$this->data['expire'] = $product_info['expire'];;//wagner validade padrão
			$this->data['tipo'] = $product_info['tipo'];;//wagner tipo
			$this->data['alerta_amarelo'] = $product_info['alerta_amarelo'];;//wagner alerta_amarelo
			$this->data['alerta_vermelho'] = $product_info['alerta_vermelho'];;//wagner alerta_vermelho
			$this->data['stock'] = $resultsStock; //wagner quantidade lotes
		}
