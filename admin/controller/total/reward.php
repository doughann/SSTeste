...
class ControllerTotalReward extends Controller { 
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			//wagner - aqui gravar intervalos
			$this->db->query("DELETE FROM " . DB_PREFIX . "reward_interval");//wagner
			$grava = $this->request->post;
			//echo "<pre>";
			//print_r($grava);
			//echo "</pre>";
			if (isset($grava['intervalo_lot'])) {

				foreach ($grava['intervalo_lot'] as $intervalo_lot) {
					
					if($intervalo_lot['pontos']>0){
						$this->db->query("INSERT INTO " . DB_PREFIX . "reward_interval SET valor_inicial = '" . $intervalo_lot['inicial']. "', valor_final = '" . $intervalo_lot['final'] . "', pontos = '" . $intervalo_lot['pontos']."'");
					}
					
				}
			}
			//fim gravaçao de intervalos
		...
		$this->data['intervalos'] = $this->db->query("SELECT * FROM `" . DB_PREFIX . "reward_interval`")->rows;
