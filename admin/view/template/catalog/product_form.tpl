<?php echo $header;
$hoje = date('Y-m-d');
if(!isset($expire)){$expire=90;}
if(!isset($alerta_amarelo)){$alerta_amarelo=5;}
if(!isset($alerta_vermelho)){$alerta_vermelho=2;}

$venc = date('Y-m-d', strtotime("+".$expire." days"));

function formatadata($dataf){//funçao wagner
	return (substr($dataf,8,2)."/".substr($dataf,5,2)."/".substr($dataf,0,4));
...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
      <div id="tabs" class="htabs"><a href="#tab-general"><?php echo $tab_general; ?></a><a href="#tab-data"><?php echo $tab_data; ?></a><a href="#tab-links"><?php echo $tab_links; ?></a><a href="#tab-attribute"><?php echo $tab_attribute; ?></a><a href="#tab-option"><?php echo $tab_option; ?></a><a href="#tab-profile"><?php echo $tab_profile; ?></a><a href="#tab-discount"><?php echo $tab_discount; ?></a><a href="#tab-special"><?php echo $tab_special; ?></a><a href="#tab-image"><?php echo $tab_image; ?></a><!--<a href="#tab-reward"><?php echo $tab_reward; ?></a>--><a href="#tab-design"><?php echo $tab_design; ?></a><a href="#tab-stock"><?php echo $tab_stock; ?></a></div>
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        ...
        <div id="tab-data">
          <table class="form">
              ...
              <td>
				<select name="tipo">
					<?php if(!isset($tipo)){$tipo=0;}?>
					 <option value="0" <?php if ($tipo=="0") { echo " selected=\"selected\" ";} ?>>outros</option>
					 <option value="1" <?php if ($tipo=="1") { echo " selected=\"selected\" ";} ?>>G</option>
					 <option value="2" <?php if ($tipo=="2") { echo " selected=\"selected\" ";} ?>>P</option>
					 <option value="3" <?php if ($tipo=="3") { echo " selected=\"selected\" ";} ?>>Mistura</option>
           ...
           <!-- <tr>
              ...
              <td><input type="text" name="quantity_ONLYSHOW" value="<?php echo $quantity; ?>" size="2" readonly disabled=disabled/></td>
            </tr> -->
            <tr>
		...
		<div id="tab-stock"><strong>Produto:</strong><?php echo isset($product_description[$language['language_id']]) ? $product_description[$language['language_id']]['name'] : ''; ?><br>
		Dias de validade padrão: <input type="text" name="expire" value="<?php if(isset($expire)){echo $expire;}?>" /><br>
		Estoque alerta amarelo: <input type="text" name="alerta_amarelo" value="<?php if(isset($alerta_amarelo)){echo $alerta_amarelo;}?>" /> unidades<br>
		Estoque alerta vermelho: <input type="text" name="alerta_vermelho" value="<?php if(isset($alerta_vermelho)){echo $alerta_vermelho;}?>" />unidades<br>
  <!-- Campo Localizaçao -->
	 <br>
		<table >
			 <thead>
		...
		<br>
            ...
            <thead>
              <tr>
                ...
                <td class="left">Data do Lote</td>
                <td class="left">Data de Validade</td>
			...
			<?php $stockCount = 0;?>
			<?php if(isset($stock)){?>
				<?php foreach ($stock as $rowStock) { ?>
				<?php $stockCount++;
				if($rowStock['stock_qty']>0){?>
				  <tr id="stock-row<?php echo $stockCount;?>">
					<td class="left"><input type="text" name="stock_lot[<?php echo $stockCount; ?>][qtd]" value="<?php echo $rowStock['stock_qty']; ?>" size="3" /> </td>
					<td class="left"><input type="text" name="stock_lot[<?php echo $stockCount; ?>][datalot]" value="<?php echo $rowStock['lot_date']; ?>" class="date" /></td>
					<td class="left"><input type="text" name="stock_lot[<?php echo $stockCount; ?>][lot_exp]" value="<?php echo $rowStock['lot_exp']; ?>" class="date" /></td>
					<!--<td class="left"> <a onclick="$('#stock-row<?php echo $stockCount ?>').remove()" class="button">Remover</a></td>-->
				  </tr>
				<?php }else{?>
					<input type="hidden" name="stock_lot[<?php echo $stockCount; ?>][qtd]" value="<?php echo $rowStock['stock_qty']; ?>" />
					<input type="hidden" name="stock_lot[<?php echo $stockCount; ?>][datalot]" value="<?php echo $rowStock['lot_date']; ?>" />
					<input type="hidden" name="stock_lot[<?php echo $stockCount; ?>][lot_exp]" value="<?php echo $rowStock['lot_exp']; ?>"  />
					<?php }
            ...
            <tfoot>
                    <tr>
                        <td colspan="3"></td>
                        <td class="left"><a onclick="addStock()" class="button">Adicionar</a></td>
                    </tr>
                </tfoot>
          </table>
        </div>
...
var stockCount = <?php echo $stockCount ?>;
var hoje = "<?php echo $hoje ?>";
...
function addStock() {
    ...
    html += '<tr id="stock-row' + stockCount + '" style="background-color: #ffd766;">';
    html += '  <td class="left"  style="background-color: #ffd766;">';
    html += '    <input type="text" name="stock_lot[' + stockCount + '][qtd]" value="1" size="3" />';
    html += '  </td>';
    html += '  <td class="left"  style="background-color: #ffd766;">';
    html += '    <input type="text" name="stock_lot[' + stockCount + '][datalot]" value="'+hoje+'" class="date" />';

    html += '  </td>';
	html += '  <td class="left"  style="background-color: #ffd766;">';
    html += '    <input type="text" name="stock_lot[' + stockCount + '][lot_exp]" value="'+venc+'" class="date" />';

    html += '  </td>';
    html += '  <td class="left"  style="background-color: #ffd766;">';
    html += '    <a class="button" onclick="$(\'#stock-row' + stockCount + '\').remove()"><?php echo $button_remove ?></a>';
    html += '  </td>';
    html += '</tr>';

    $('#tab-stock table tbody').append(html);

	$('#stock-row' + stockCount + ' .date').datepicker({dateFormat: 'yy-mm-dd'});
}
